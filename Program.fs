...
let problem = function
    | i when not (List.exists (fun (number,_,_) -> number = i) problems) ->
        printfn "%s: There is no such problem number." (i |> string)
    | i ->
...
let loop () =
    ...
    let run = function
        | Prefix "test" number | Prefix "t" number when Int32.TryParse(number) |> fst ->
            Int32.TryParse(number) |> snd |> problem
            false
        | "test" | "t" ->
            printfn "You have to specify problem number."
            false
        | Prefix "source" number | Prefix "s" number (*when Int32.TryParse(number) |> fst*) ->
            // Int32.TryParse(number) |> snd |> source
